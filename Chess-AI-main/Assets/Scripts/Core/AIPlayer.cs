namespace Chess.Game {
	...
	public class AIPlayer : Player {
		...
		public AIPlayer (Board board, AISettings settings, ISearch search) {
			this.settings = settings;
			this.board = board;
			settings.requestAbortSearch += TimeOutThreadedSearch;
			this.search = search;
			search.onSearchComplete += OnSearchComplete;
			search.Diagnostics = new SearchDiagnostics ();
		...
		public override void Update () {
			...
			settings.diagnostics = search.Diagnostics;

		}

		public override void NotifyTurnToMove () {

			search.Diagnostics.isBook = false;
			moveFound = false;
			...
			if (bookMove.IsInvalid) {
				...
				search.Diagnostics.isBook = true;
				search.Diagnostics.moveVal = Chess.PGNCreator.NotationFromMove (FenUtility.CurrentFen(board), bookMove);
				settings.diagnostics = search.Diagnostics;
