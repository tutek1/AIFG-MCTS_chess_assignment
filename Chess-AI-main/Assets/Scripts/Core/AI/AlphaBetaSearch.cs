namespace Chess {
	...
	public class AlphaBetaSearch : ISearch {
			...
			if (settings.useIterativeDeepening) {
				...
				for (int searchDepth = 1; searchDepth <= targetDepth; searchDepth++) {
					...
					if (abortSearch) {
						...
						Diagnostics.moveVal = Chess.PGNCreator.NotationFromMove (FenUtility.CurrentFen (board), bestMove);
		...
		int SearchMoves (int depth, int plyFromRoot, int alpha, int beta) {
			...
			List<Move> moves = moveGenerator.GenerateMoves_DO_NOT_USE (board);
		...
		int QuiescenceSearch (int alpha, int beta) {
			...
			var moves = moveGenerator.GenerateMoves_DO_NOT_USE (board, false);
